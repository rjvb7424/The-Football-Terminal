...
export default function SituationMapPanel({ height = { xs: 520, md: 650 }, listMaxHeight = 650 }: Props) {
  ...
  const focusPoint = (point: SituationPoint) => {
    ...
    mapRef.current?.flyTo([point.lat, point.lng], 6.25, { duration: 1, easeLinearity: 0.16 });
  };

  return (
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: '1fr 330px' }, gap: 3 }}>
      <Box
        sx={{
          height,
      ...
      >
        ...
        >
          <Typography sx={{ fontSize: '0.62rem', color: C.accent, fontFamily: 'monospace', fontWeight: 850, letterSpacing: '0.08em' }}>
            EUROPE FOCUS · GLOBAL SIGNALS
          </Typography>
        </Box>
      </Box>

      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.35, maxHeight: { xl: listMaxHeight }, overflow: { xl: 'auto' } }}>
        ...
        {SITUATION_POINTS.map(point => (
          ...
          >
            <Box sx={{ minWidth: 0 }}>
              <Typography sx={{ fontSize: '0.77rem', color: C.text1, fontWeight: 760, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {point.event}
              </Typography>
              <Typography sx={{ fontSize: '0.65rem', color: C.text3, mt: 0.25 }}>
                {point.club} · {point.city}
              </Typography>
            </Box>
